...
export const Route = createFileRoute("/admin")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Piloter la fréquence de scraping et suivre les exécutions de la veille." },
    ],
  }),
  component: Admin,
});
...
function Admin() {
  ...
  const { data: logs = [], isLoading: loadingLogs } = useQuery({ queryKey: ["scrapeLogs"], queryFn: () => getScrapeLogs(30) });
  ...
  const saveFreq = async () => {
    ...
    try {
      ...
      setMsg({ kind: "ok", text: `Fréquence mise à jour : ${FREQUENCY_LABELS[effectiveSelected as ScrapeFrequency]}.` });
    } catch (e) {
    ...
    return (
      <div className="max-w-3xl">
        ...
        <div className="card-flat p-6 text-sm text-muted">
          Cette page pilote le scraping automatique hébergé sur Supabase. Elle nécessite une connexion Supabase active
          (mode actuel : <span className="font-medium text-text">local</span>).
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl space-y-6">
      <header>
        ...
        <div className="text-sm text-muted mt-1">Pilotez la veille automatique et suivez ses exécutions.</div>
      </header>

      {msg && (
        <div className={`flex items-start gap-2 text-sm px-3 py-2 rounded-md ${msg.kind === "ok" ? "bg-[#E8F5F0] text-emerald-800" : "bg-[#FFF4E6] text-orange-700"}`}>
          {msg.kind === "ok" ? <CheckCircle2 className="w-4 h-4 shrink-0 mt-0.5" /> : <XCircle className="w-4 h-4 shrink-0 mt-0.5" />}
          <span>{msg.text}</span>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="card-flat p-5">
          ...
          <p className="text-xs text-muted mb-4">
            Le robot interroge le portail EU Funding &amp; Tenders et met la base à jour automatiquement.
            {freq?.cron && <> Expression cron actuelle : <code className="text-[11px]">{freq.cron}</code> (UTC).</>}
          </p>
          ...
          <p className="text-[11px] text-muted mt-1.5">
            ...
            <span className="font-medium text-text">{currentFreq ? FREQUENCY_LABELS[currentFreq] : "non planifié"}</span>.
          </p>
          <button
            onClick={saveFreq}
            disabled={savingFreq}
          ...
          >
            {savingFreq ? <Loader2 className="w-4 h-4 animate-spin" /> : <CalendarClock className="w-4 h-4" />}
        ...
        <div className="card-flat p-5">
          ...
          <p className="text-xs text-muted mb-4">
            Lance immédiatement une récupération des appels à projets, sans attendre la prochaine exécution planifiée.
          </p>
          <button
            onClick={runNow}
            disabled={running}
      ...
      <div className="card-flat p-5">
        ...
        {loadingLogs ? (
          ...
          <div className="text-sm text-muted py-6 text-center">Aucune exécution enregistrée pour l'instant.</div>
        ) : (
                  ...
                  <tr key={l.id} className="text-text">
                    ...
                    <td className="py-2 px-3">
                      {l.ok ? (
                        ...
                        <span className="inline-flex items-center gap-1 text-orange-700" title={l.error ?? ""}><XCircle className="w-3.5 h-3.5" /> Échec</span>
                      )}
                    </td>
                    <td className="py-2 px-3 text-right tabular-nums">{l.fetched}</td>
                    <td className="py-2 px-3 text-right tabular-nums font-medium text-navy">{l.nouveaux}</td>
                    <td className="py-2 px-3 text-right tabular-nums">{l.mis_a_jour}</td>
                    <td className="py-2 px-3 text-right tabular-nums">{l.fermes}</td>
                    <td className="py-2 pl-3 text-right tabular-nums text-muted">{l.duration_ms != null ? `${(l.duration_ms / 1000).toFixed(1)} s` : "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
